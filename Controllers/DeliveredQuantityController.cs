    ...
    public class DeliveredQuantityController : Controller
    {
        ...
        public async Task<IActionResult> Save([FromBody] DeliveredQuantitySaveModel model)
        {
            try
            {
                ...
                // Save or update DealerOutstanding
                ...
                if (dealerOutstanding == null)
                {
                    dealerOutstanding = new DealerOutstanding
                    {
                        ...
                        PaidAmount = 0, // Assuming initial paid amount is 0
                        BalanceAmount = grandTotal // Assuming initial balance is grandTotal
                    };
                    await _context.DealerOutstandings.AddAsync(dealerOutstanding);
                }
                else
                {
                    dealerOutstanding.InvoiceAmount = grandTotal;
                    dealerOutstanding.BalanceAmount = grandTotal - dealerOutstanding.PaidAmount; // Recalculate balance
